...
import we.retail.core.services.SolrSearchConfigurationService;

@Component(immediate = true, service = SolrServerConfiguration.class, configurationPolicy = ConfigurationPolicy.REQUIRE)
@Designate(ocd = SolrSearchConfigurationService.class)
public class SolrServerConfigurationImpl implements SolrServerConfiguration
{
    ...
    @Activate
    @Modified
    public void activate(final SolrSearchConfigurationService config)
    {
